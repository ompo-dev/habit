...
(async () => {
  ...
  try {
    // Tentar acessar o store via localStorage
    ...
    if (habitsStorage) {
      ...
      if (habits.length > 0) {
        console.log(
          "📋 Hábitos:",
          habits.map((h) => h.title || h.name).join(", ")
        );
      }
    } else {
  ...
  try {
    ...
    if (db) {
      ...
      console.log(
        "ℹ️ IndexedDB ainda não foi criado (normal se SW não foi ativado)"
      );
    }
  } catch (e) {
  ...
  try {
    ...
    notification1.onerror = (e) =>
      console.error("❌ Erro na notificação básica:", e);
  ...
  if (registration && registration.active) {
    try {
      // Testar UPDATE_REMINDER_PREFERENCES
  ...
  if (registration && registration.active) {
    try {
      ...
      console.log(
        "💡 Se houver hábitos não completados, você deve receber notificações"
      );
    } catch (e) {

...
const POPULAR_ICONS = [
  "Droplet", "Coffee", "BookOpen", "Code", "Dumbbell", "Cookie", "Brain", "Languages",
  "Pill", "Stretch", "Sparkles", "DollarSign", "Heart", "Footprints", "Cigarette",
  "Smartphone", "Apple", "Layout", "Moon", "Music", "Mail", "Hourglass", "Origami",
  "PenTool", "Target", "Sun", "Zap", "Star", "Camera", "Leaf", "Flame", "Trophy",
  "Clock", "Calendar", "CheckCircle", "Activity", "TrendingUp", "BarChart", "Award",
  "Bike", "Book", "Briefcase", "BellRing", "Pizza", "Utensils", "Gamepad2", "Laptop",
]
...
export function HabitCustomizationModal({ habitId, isOpen, onClose }: HabitCustomizationModalProps) {
  ...
  return (
    <AnimatePresence>
      {isOpen && habit && (
        ...
        {/* Header */}
        <div className="flex items-center justify-between mb-6 sticky top-0 bg-background pb-4 border-b border-white/10">
          <h2 className="text-2xl font-bold text-white">Personalizar Hábito</h2>
          <button
            onClick={onClose}
        ...
        <div className="mb-6 p-6 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl shadow-lg">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {IconComponent && <IconComponent className="h-8 w-8" style={{ color: selectedColor.primary }} />}
            </div>
            <div>
              <h3 className="text-xl font-bold text-white">{title || "Nome do hábito"}</h3>
        ...
        {/* Nome */}
        <div className="mb-6">
          <label className="block text-sm font-medium text-white mb-2">Nome do Hábito</label>
          <input
            type="text"
            value={title}
        ...
        {/* Ícones */}
        <div className="mb-6">
          <label className="block text-sm font-medium text-white mb-3">Ícone</label>
        ...
        {/* Cores */}
        <div className="mb-6">
          <label className="block text-sm font-medium text-white mb-3">Cor</label>
          <div className="grid grid-cols-4 sm:grid-cols-6 gap-3">
            {COLORS.map((color) => {
              ...
              return (
                <button
                  key={color.name}
                  onClick={() => setSelectedColor({ primary: color.primary, background: color.background })}
                  className={cn(
